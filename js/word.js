  ...
  follow_adjacencies(current_game, orientation, tile, dont_follow) {
    ...
    if (orientation == Word.ORIENTATIONS.HORIZ) {
      ...
      while (t) {
        ...
        if (!t.is_safe) {
          current_game.update_play(t);
          if ( this.player != t.player) {
            t.push_state(new Tile.TileState(t, Tile.TileState.setplayer, this.play));
            t.player = this.player;
          }
          if (this.is_safe && !dont_follow) {
            // t.set_tile_color(t.player.tile_color_safe);
            // this is for rolling back a play
            t.push_state(new Tile.TileState(t, Tile.TileState.setissafe, this.play));
            t.is_safe = true;
          } else {
            // t.set_tile_color(t.player.tile_color_risky);
            t.is_safe = false;
          }
        }
      ...
      while (t) {
        ...
        if (!t.is_safe){
          ...
          // safety belongs to the word (this) - so, if we're following
          // an orthogonal we can change ownership (player) but not
          // safety.
          if (this.is_safe && !dont_follow) {
            // t.set_tile_color(t.player.tile_color_safe);
            t.push_state(new Tile.TileState(t, Tile.TileState.setissafe, this.play));
